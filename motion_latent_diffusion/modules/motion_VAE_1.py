...
class TransformerMotionAutoencoder(pl.LightningModule):
    def __init__(
        ...
        **config,
        ...
        self.hindden_encoder_layer_widths = config.get(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if current_epoch not in self.epochs_animated:
            ...
            recon = res["recon"]
            x = res["motion_seq"]
            idx = np.random.randint(0, recon.shape[0])
            im_arr = plot_3d_motion_frames_multiple(
                [recon[idx].cpu().detach().numpy(), x[idx].cpu().detach().numpy()],
                ["recon", "true"],
                nframes=5,
                radius=2,
                figsize=(20, 8),
            ...
            if self.save_animations:
                ...
                fname = f"{self.subfolder}/recon_epoch{current_epoch}.mp4"
                plot_3d_motion_animation(
                    recon[idx].cpu().detach().numpy(),
                    "recon",
                    figsize=(10, 10),
                    fps=20,
                    radius=2,
                ...
                # copy file to latest
                shutil.copyfile(fname, f"{self.folder}/recon_latest.mp4")

                if current_epoch == 0:
                    plot_3d_motion_animation(
                        x[idx].cpu().detach().numpy(),
                        "true",
                        figsize=(10, 10),
                        fps=20,
                        radius=2,
                        save_path=f"{self.folder}/recon_true.mp4",
                        velocity=False,
                    )
                    plt.close()
    ...
    def test_step(self, batch, batch_idx):
        ...
        if batch_idx == 1 and self.save_animations:
            ...
            print("Saving animations")
            plot_3d_motion_animation(
                recon[3].cpu().detach().numpy(),
                "recon",
                figsize=(10, 10),
                fps=20,
                radius=2,
                save_path=f"{self.folder}/recon_test.mp4",
                velocity=False,
            )
            plt.close()
        return loss
    ...
    def _common_step(self, batch, batch_idx, verbose=False):
        #pose0, velocity_relative, root_travel, 
        motion_seq, text = batch
        # motion_less_root = motion_seq - root_travel  # relative motion
        # root_less = True
        # if root_less:
        #     recon, mu, logvar, z = self(motion_less_root)
        #     motion_less_root_rec = recon
        # else:
        motion_less_root = motion_seq - motion_seq[:, :1]
        ...
        pose0, velocity_relative, root_travel = self.decompose_recon(motion_seq) 
        ...
        loss_data = {
            ...
            "motion_relative": {
                "true": motion_less_root,
                "rec": motion_less_root_rec,
            },  #'weight':       100},
        }
